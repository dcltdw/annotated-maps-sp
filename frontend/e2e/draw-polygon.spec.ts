...
// ---------------------------------------------------------------------------
// Draw → save e2e for polygon (area) notes.
//
...
// MapScreen sets drawMode="polygon" → MapView calls drawer.startDraw(mode, cb),
// our fake captures `cb` and exposes `window.__emitShape(coords)` so the test can
// fire it to simulate "finished the polygon". From there it's the real app: the
// NoteEditor opens (create mode) carrying the shape, Save POSTs the note with the
// shape, and a stateful re-fetch renders it via the A2.1 `regions` source.
// ---------------------------------------------------------------------------
...
// The polygon ring the fake will "draw" (a closed [lng,lat] ring).
...
test.describe("draw polygon (injected fake drawer)", () => {
  test("draw → editor → save POSTs a polygon shape and renders the region", async ({ page }) => {
    ...
    // Select a non-guest persona so canWrite is true and "Draw area" appears.
    await page.getByRole("button", { name: "You (owner)" }).click();

    // Start drawing.
    await page.getByRole("button", { name: /draw area/i }).click();

    // Simulate finishing the polygon — fires the captured onComplete.
    await page.evaluate(
    ...
    await titleInput.fill("My Drawn Area");
    await page.getByLabel("Section content").first().fill("area section content");
